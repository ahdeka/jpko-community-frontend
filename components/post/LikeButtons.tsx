  ...
  return (
    ...
    <div className="flex gap-3 justify-center py-6">
      <button
        ...
        aria-label="좋아요"
        className={`flex min-w-[104px] items-center justify-center gap-1.5 rounded-full border px-6 py-2.5 text-sm font-medium shadow-sm transition-colors ${
          myType === 'LIKE'
      ...
      >
        ...
        <span className="font-semibold text-emerald-600 dark:text-emerald-400">{likeCount}</span>
      </button>
      <button
      ...
      >
        ...
        <span className="font-semibold text-red-500 dark:text-red-400">{dislikeCount}</span>
      </button>
    </div>
  )
}
